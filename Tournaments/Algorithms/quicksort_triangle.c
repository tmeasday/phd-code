...
{
	digraph sub = subdigraph (dg, nodes, n);
	
	int* counts = get_vertex_triangle_counts (sub);
	
	int best_counts = counts[0], best = nodes[0];
	int i; for (i = 0; i < n; i += 1) {
		if (counts[i] < best_counts) {
			best_counts = counts[i];
			best = nodes[i];
		}
	}
