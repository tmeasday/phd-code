...
solve_digraph (digraph dg)
{
  ...
  CPXLPptr lp;
  lp = CPXcreateprob (env, &status, NULL);
...
#else
      obj[col_no-1] = dg_weight (dg, i, j);
      lb[col_no-1] = 0.0;
      ub[col_no-1] = CPX_INFBOUND;
#endif
			col_no += 1;
		}
	}
...
#ifdef GLPK
			glp_set_row_bnds (lp, row_no+1, GLP_FX, 1.0, 1.0);
			
			int    is[] = {0, col_nos[i][j], col_nos[j][i]};
			double as[] = {0, 1, 1};
			glp_set_mat_row  (lp, row_no+1, 2, is, as);
#else
      rhs[row_no]         = 1.0;
      sense[row_no]       = 'E';
      matbeg[row_no]      = cntno;
...
#ifdef GLPK	
				glp_set_row_bnds (lp, row_no+1, GLP_LO, 0.0, 0.0);
